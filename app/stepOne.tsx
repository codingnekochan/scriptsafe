...
const StepOne = () => {
  ...
  const [comorbiditiesMedicationList, setComorbiditiesMedicationList] =
    useState([]);
  ...
  useEffect(() => {
    if (
      ...
      const medicationData = fetchPatientData[3]?.data?.data?.data.map(
        (item: any) => ({
          id: item.id,
          name: item.name,
          dosage: "0mg",
      ...
      setComorbiditiesMedicationList(medicationData);
    }
    if (
